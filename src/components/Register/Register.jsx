...
const Register = () => {
    ...
    const validate = () => {
        ...
        if (!name) {
            ...
            newError.name = "Name must be at least 3 charaters";
            valid = false;
        }
        if (!email) {
    ...
    const handleSubmit = (e) => {
        ...
        if (res.success) {
            ...
            if (res.message && res.message.toLowerCase().includes("name")) {
                ...
                setError({ name: "", email: "", password: "", confirmPassword: res.message || "Register failed" });
            }
        }
    };
    ...
    return (
        <div className="flex min-h-screen items-center relative overflow-hidden justify-center">
            ...
            <div className="relative z-10 w-full max-w-md mx-2 p-10 rounded-2xl shadow-2xl bg-white/30 backdrop-blur-lg border border-white/40">
                ...
                <h2 className="text-3xl font-extrabold mb-2 text-gray-900 text-center drop-shadow">Create your account</h2>
                <p className="text-base text-gray-700 mb-8 text-center">Sign up to get started with your account</p>
                <form className="space-y-5" onSubmit={handleSubmit}>
                    <div>
                        <label className="block text-sm font-semibold mb-1 text-gray-800" htmlFor="name">Full Name</label>
                        <input
                            id="name"
                            type="text"
                    ...
                    <div>
                        <label className="block text-sm font-semibold mb-1 text-gray-800" htmlFor="password">Password</label>
                    ...
                    <div>
                        <label className="block text-sm font-semibold mb-1 text-gray-800" htmlFor="confirmPassword">Confirm Password</label>
                        <div className="relative">
                            <input
                                id="confirmPassword"
                ...
                <p className="text-center text-sm text-gray-700">
                    ...
                    <Link to={"/login"} className="text-black hover:underline font-semibold">Login</Link>
                </p>
            </div>
        </div>
    );
